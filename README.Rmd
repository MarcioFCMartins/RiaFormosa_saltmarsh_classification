...
**this whole area is a very initial draft**
...
### Tiling system
In order to work with larger-than-memory rasters, I used a custom approach: split the raster into . While it did work, it doesn't scale too well and suffers from many downsides (too slow, inneficient data storage format). I used a tiling system (based on this talk - ADD REFERENCE, LARGER THAN MEMORY RASTER HANDLING FORM THE RGIS WORKSHOP) which can be used to read in tiles one by one or, if this process needs to be repeated, can be used to save .rds files for speed. More testing is needed for optimization. See an example of

> TODO: look into using hashes for filenames so that repeated tiles are never saved again. Add a function that skips tiles where all cells are NA

You can see an example of the result of this tiling system below. Also, some of my attempts at image processing, contrast enhancing and image segmentation.
```{r}
